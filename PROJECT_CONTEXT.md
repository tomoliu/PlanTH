...
- **Autoload**：`ConfigManager`（显示设置 + 输入 + GM）+ `SaveManager`（存档 3 槽）
...
| 玩家 hitbox | 半径 20px |
| 敌人速度 | 依属性表（默认 240 px/s，八向 AI 靠近玩家） |
| 敌人 hitbox | 半径 20px |
| 敌人射击间隔 | 依属性表（默认 2s） |
| 子弹速度 | 250 px/s |
| 子弹半径 | 10px |
...
### 敌人系统
- 数量从 `敌人配置表` 读取，难度 0 = 1 个敌人
- 敌人属性从 `敌人属性表` 读取（移速、体力、射击间隔、碰撞伤害、子弹速度）
- 敌人环形分布生成，AI 八向移动靠近玩家
- 敌人碰触玩家 → 扣 HP + 弹开；HP=0 → 死亡

### 玩家体力
- 从角色选择传递（灵梦 6，魔理沙 5）
- 左上角 HPLabel 实时显示 ❤ 当前/最大
...
| 路径 | 说明 |
|------|------|
| `data/tables/角色表.xlsx` → `data/json/角色表.json` | ID/名称/大厅背景/初始体力/初始金钱 |
| `data/tables/敌人配置表.xlsx` → `data/json/敌人配置表.json` | 难度(0-10)/敌人数量/条件描述 |
| `data/tables/敌人属性表.xlsx` → `data/json/敌人属性表.json` | 敌人ID/名称/移速/体力/射击间隔/碰撞伤害 |

转换命令：`python3 xls2json/convert_*.py`
...
| 文件 | 说明 |
|------|------|
| `xls2json/convert_characters.py` | Excel 角色表 → JSON |
| `xls2json/convert_enemies.py` | Excel 敌人配置 → JSON |
| `xls2json/convert_enemy_attrs.py` | Excel 敌人属性 → JSON |
| `xls2json/read_xlsx.py` | Excel 读取辅助 |

运行：`python3 xls2json/convert_characters.py && python3 xls2json/convert_enemies.py && python3 xls2json/convert_enemy_attrs.py`
